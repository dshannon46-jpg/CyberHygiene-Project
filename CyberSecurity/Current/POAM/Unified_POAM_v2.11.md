...
**Reference:** System Security Plan Version 2.9
...
- Previous Score: 101/110 (91.8%) as of February 21, 2026
- Target Score: 110/110 (100%)
- Target Completion: April 30, 2026 (risk assessment), June 30, 2026 (IR testing)

SPRS score revised per independent CMMC L2 Preliminary Gap Analysis (February 9, 2026). See Assessment Reference section below. Score updated 02/21/2026 after completion of multi-factor authentication deployment (POA&M-004, +5 points). MFA now fully implemented on all 4 CPN systems: dc1, labrat, engineering, and accounting.
...
- High Priority: 9 items (6 completed, 2 on track, 1 planned)
...
| POA&M-009 | File integrity monitoring not deployed | SI-7 | 10/28/2025 | Wazuh FIM operational with 12-hour scan intervals on critical paths. **02/21/2026 operational update:** VirusTotal integration tuned — alert level threshold raised 7→10 (routine FIM events no longer trigger VT lookups; only high-severity events such as executable modifications). MD5 hash deduplication cache and 490/day quota guard added to virustotal.py to prevent free-tier exhaustion (root cause: 2,500+ level-7 syscheck alerts/day across 4 agents). Integration remains active and operational. No SPRS impact. | COMPLETED |
...
#### Multi-Factor Authentication (February 21, 2026)

| POA&M ID | Weakness/Deficiency | NIST Controls | Completion Date | Evidence | Status |
|----------|---------------------|---------------|-----------------|----------|--------|
| POA&M-004 | Multi-factor authentication not configured | 3.5.3 (IA-2(1), IA-2(2)), IA-8 | 02/21/2026 | SSH publickey + TOTP (pam_google_authenticator) deployed on all 4 CPN systems. Microsoft Authenticator. DC1 admin exemption via pam_succeed_if.so rhost. **SPRS: +5 points recovered (3.5.3: +5). Absorbs POA&M-SPRS-1 (IA-8).** | COMPLETED |

**POA&M-004 (MFA) Completion Evidence:**

Multi-factor authentication (NIST 800-171 3.5.3) fully implemented on all CPN systems as of 02/21/2026:

- **Authentication method:** SSH public key (ECDSA-521) + TOTP via `pam_google_authenticator.so` (time-based, 30-second window, SHA1, 6-digit)
- **Authenticator app:** Microsoft Authenticator (TOTP-compatible, RFC 6238)
- **PAM configuration:** `/etc/pam.d/sshd` — `auth sufficient pam_succeed_if.so quiet rhost = 192.168.1.10` (dc1 admin exemption) + `auth required pam_google_authenticator.so nullok`
- **SSH configuration:** `/etc/ssh/sshd_config.d/60-mfa.conf` — `AuthenticationMethods publickey,keyboard-interactive`; `KbdInteractiveAuthentication yes`
- **SELinux:** Custom policy module `sshd_google_auth` deployed on all systems — allows `sshd_t` to access `auth_home_t` (`.google_authenticator` file)
- **DC1 admin exemption:** `pam_succeed_if.so rhost = 192.168.1.10` with `sufficient` flag — automated admin scripts from dc1 (192.168.1.10) bypass TOTP; all other connections require TOTP
- **Systems deployed:** dc1 (02/21/2026), labrat (02/21/2026), engineering (02/21/2026), accounting (02/21/2026) — all 4 CPN systems
- **Engineering note:** Bitdefender Security Tools (`bdsecd`) was proxying SSH connections and masking source IPs — removed 02/21/2026, enabling correct pam_succeed_if dc1 IP exemption
- **NIST controls addressed:** 3.5.3 (IA-2(1), IA-2(2)) — MFA for privileged and non-privileged accounts; IA-8 — organizational and non-organizational users (absorbed from POA&M-SPRS-1)
- **Scratch codes:** Stored in KeePass password vault per key management policy (TCC-IAP-001)
...
| POA&M ID | Weakness/Deficiency | NIST Controls | Completion Date | Evidence | Status |
|----------|---------------------|---------------|-----------------|----------|--------|
| POA&M-011 | Security control assessment not formally established | 3.12.1 (CA-2) | 02/17/2026 | **Formal Assessment:** Independent CMMC L2 Preliminary Gap Analysis conducted 02/09/2026 by C3PAO reviewer covering all 110 NIST 800-171 requirements. **Ongoing Automated Assessment:** OpenSCAP CUI profile (xccdf_org.ssgproject.content_profile_cui) scans all 4 CPN systems (dc1, labrat, engineering, accounting) every 12 hours via systemd timers. Centralized daily collection via `collect_openscap_results.sh` with consolidated HTML compliance dashboard at https://dc1.cyberinabox.net/dashboard/openscap-dashboard.html. Per-system pass/fail/score tracking with drill-down to full OpenSCAP HTML reports. Markdown summary at CyberSecurity/Current/Assessments/OpenSCAP/CUI_Compliance_Summary_YYYYMMDD.md. Assessment scope: all SCAP Security Guide CUI rules mapped to NIST 800-171 controls. **SPRS: +3 points recovered (3.12.1: +3).** **02/21/2026 milestone:** All 4 CPN systems achieved 100% OpenSCAP CUI compliance simultaneously for the first time — dc1 (104/104), labrat (104/104), engineering (104/104), accounting (104/104). Previously failing rules resolved: `sshd_disable_root_login` (PermitRootLogin no set on all workstations; admin moved to dshannon+sudo) and `sysctl_user_max_user_namespaces` (kernel parameter disabled via /etc/sysctl.d/99-user-namespaces.conf on all workstations). Collection script updated to use `dshannon@host` with NOPASSWD sudo — no longer requires root SSH. No SPRS impact (3.12.1 already credited). | COMPLETED |

#### Technical Controls (dc1)

| POA&M ID | Weakness/Deficiency | NIST Controls | Completion Date | Evidence | Status |
|----------|---------------------|---------------|-----------------|----------|--------|
| POA&M-029 | Session lock not configured (15-min timeout) | 3.1.10 (AC-11) | 02/15/2026 | GNOME dconf: idle-delay=900s, lock-enabled=true, settings locked. Config: /etc/dconf/db/local.d/01-screensaver. **Deployed: dc1, labrat, engineering, accounting. All 4 CPN systems fully deployed as of 02/21/2026.** | COMPLETED |
| POA&M-034 | Login banners not implemented on all systems | 3.1.9 (AC-8) | 02/15/2026 | Login banner in /etc/issue and /etc/issue.net. SSH Banner configured. Verified: `sshd -T \| grep banner`. **Deployed: dc1, labrat, engineering, accounting. All 4 CPN systems fully deployed as of 02/21/2026.** **02/21/2026 banner update:** Banner text changed from USG/DoD government-system notice to CyberHygiene Project Contractor Information System (CIS) access notice — system is a commercial contractor IS, not a government system. New text: "You are accessing a CyberHygiene Project Contractor Information System (CIS) that is provided for company-authorized use only." Updated on all 4 systems and in bootstrap scripts (ws-setup.sh, fix-engineering.sh). No SPRS impact (3.1.9 remains MET). | COMPLETED |
| POA&M-044 | Removable media technical enforcement (USBGuard) | 3.8.7 (MP.L2-3.8.7) | 02/15/2026 | USBGuard 1.0.0-16.el9 with device-specific allow policy per host. Wazuh monitoring via /var/log/usbguard/usbguard-audit.log. Verified: `usbguard list-devices`. **Deployed: dc1, labrat, engineering, accounting. All 4 CPN systems fully deployed as of 02/21/2026.** | COMPLETED |
...
**POA&M-042 Operational Update (02/21/2026):**
AI inference server (ai.cyberinabox.net, 192.168.1.7) reconfigured for network accessibility:
- Ollama service bound to all interfaces (OLLAMA_HOST=0.0.0.0:11434); firewall rule added for port 11434/tcp
- Default model changed from llama3.3:70b-instruct-q4_K_M (OOM — exceeded available memory) to codellama:34b
- SysAdmin Agent Dashboard (sysadmin-agent.service) updated to use `default` model alias for resilience
- All AI traffic remains within 192.168.1.0/24 internal network; no external exposure
- Dashboard connectivity confirmed operational (✅ AI Connected)
- No SPRS impact; no change to control status (3.1.3, 3.13.1 remain MET)
...
|---------|-------------|-----------|-----------|-------|
| Q1 2026 (Jan-Mar) | 11 items | 11 | 0 | MFA completed 02/21/2026 |
...
| Identification & Authentication (IA) | 2 | 2 | 0 | — |
...
*Items assigned to primary control family. Total: 45 items, 35 completed, 10 open (4 on track, 2 ongoing, 1 planned, 1 risk accepted, 2 AI ongoing). SPRS remaining deficit: -4.*
...
| 02/21/2026 | 45 | 34 | 76% | v2.8: Operational improvements — no new completions, no SPRS impact. AI server (192.168.1.7) Ollama reconfigured for network accessibility; model changed to codellama:34b (70B OOM). SysAdmin Agent Dashboard operational. Wazuh/VirusTotal integration tuned: level 7→10, MD5 hash cache added, 490/day quota guard. |
| 02/21/2026 | 45 | 34 | 76% | v2.9: Accounting workstation returned from maintenance. CMMC controls deployed: USBGuard (3.8.7), session lock 15 min (3.1.10), DoD login banners (3.1.9). All 4 CPN systems now fully hardened. POA&M-029, 034, 044 pending caveat removed. No SPRS impact (controls already credited). |
| 02/21/2026 | 45 | 34 | 76% | v2.10: SSH hardening complete. Remote admin switched from root SSH to named user dshannon with NOPASSWD sudo (ECDSA-521 key). PermitRootLogin no on all 3 workstations. Login banner updated to CyberHygiene Project CIS notice. All 4 CPN systems first-ever 100% OpenSCAP CUI compliance (104/104 pass, 0 fail). No SPRS impact. |
| 02/21/2026 | 45 | 35 | 78% | v2.11: **MFA DEPLOYED — POA&M-004 CLOSED.** SSH publickey + TOTP (pam_google_authenticator) deployed on all 4 CPN systems. Microsoft Authenticator. Bitdefender bdsecd removed from engineering and accounting (was masking source IPs, breaking pam_succeed_if dc1 exemption). SPRS **101→106** (+5 points: 3.5.3 MFA). |
| Projected 04/30/2026 | 45 | 36+ | 80%+ | Q2 target: Risk assessment |
...
### Remaining Deficits (as of February 21, 2026)
...
**Current SPRS (post-MFA deployment):** 106/110 (96.4%)
**Target SPRS Upon Completion:** 110/110 (100%)

*Note: Session lock (3.1.10, -1), login banners (3.1.9, -1), and removable media (3.8.7, -1) implemented on all 4 systems by 02/21/2026. 3 points recovered. Training program (3.2.1, 3.2.2, 3.2.3) completed 02/17/2026, 7 points recovered. Security control assessment program (3.12.1) completed 02/17/2026, 3 points recovered. Multi-factor authentication (3.5.3) completed 02/21/2026, 5 points recovered.*
...
| 2.10 | 02/21/2026 | D. Shannon | **SSH HARDENING + FULL OPENSCAP COMPLIANCE:** Remote administration of CPN workstations switched from direct root SSH to named user `dshannon` with NOPASSWD sudo (ECDSA-521 key, /etc/sudoers.d/dshannon-admin, sudoers: files in nsswitch.conf). PermitRootLogin no enforced on all 3 workstations — resolves `sshd_disable_root_login` OpenSCAP finding. Login banner updated from USG government notice to CyberHygiene Project CIS company access notice on all 4 systems. Admin scripts updated: collect_openscap_results.sh, trigger_all_openscap_scans.sh, fix-engineering.sh, ws-setup.sh, deploy-ws-controls.sh. Result: all 4 CPN systems now 100% OpenSCAP CUI compliant (104/104 pass, 0 fail) — first time full compliance achieved across entire CPN. POA&M-011 and POA&M-034 updated with operational notes. SSP reference updated to v2.8. No SPRS impact. |
| 2.11 | 02/21/2026 | D. Shannon | **MFA DEPLOYMENT COMPLETE — POA&M-004 CLOSED:** SSH publickey + TOTP (pam_google_authenticator, time-based, Microsoft Authenticator) deployed and verified on all 4 CPN systems (dc1, labrat, engineering, accounting). AuthenticationMethods publickey,keyboard-interactive enforced via /etc/ssh/sshd_config.d/60-mfa.conf. PAM configured with dc1 admin exemption (pam_succeed_if.so rhost = 192.168.1.10, sufficient). SELinux sshd_google_auth policy module deployed. Bitdefender Security Tools (bdsecd) removed from engineering and accounting — was transparently proxying SSH connections as loopback, masking source IPs and breaking pam_succeed_if dc1 exemption. TOTP verified end-to-end on all systems. POA&M-004 moved to COMPLETED. SPRS improved 101→106 (+5 points: 3.5.3 MFA). Remaining SPRS deficit: -4 (risk assessment -3, IR testing -1). SSP reference updated to v2.9. |
| 2.9 | 02/21/2026 | D. Shannon | **ACCOUNTING WORKSTATION DEPLOYMENT:** Accounting (192.168.1.113) returned from maintenance. CMMC controls deployed via fix-engineering.sh: USBGuard (POA&M-044/3.8.7), session lock 15-min idle via GNOME dconf (POA&M-029/3.1.10), login banners in /etc/issue and /etc/issue.net (POA&M-034/3.1.9). All 4 CPN systems (dc1, labrat, engineering, accounting) now fully hardened. Pending caveats removed from POA&M-029, 034, 044. No SPRS impact — points already credited at initial deployment. |
| 2.8 | 02/21/2026 | D. Shannon | **OPERATIONAL IMPROVEMENTS (NO SPRS IMPACT):** AI server (192.168.1.7) reconfigured — Ollama bound to all interfaces, firewall port 11434/tcp opened, model changed from llama3.3:70b (OOM) to codellama:34b, SysAdmin Agent Dashboard updated to use `default` model alias. Wazuh/VirusTotal integration tuned — alert level raised 7→10, MD5 hash deduplication cache and 490/day quota guard added to virustotal.py (root cause: 2,500+ syscheck events/day exhausting 500/day free tier). SSP reference updated to v2.6. No change to item counts or SPRS score. |
